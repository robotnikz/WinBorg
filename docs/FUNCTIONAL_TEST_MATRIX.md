...
| Jobs + scheduler | Double-runs; schedule dedupe bugs; scheduled runs not reflected in the UI | `src/test/scheduler.test.ts`, `src/App.scheduledJob.integration.test.tsx` | (N/A) | `e2e/jobs.spec.ts` | `e2e/jobs.spec.ts` | (optional) |
...
| Recovery drill confidence | Restore workflow looks healthy but cannot actually recover files; status UI drifts from state | `src/utils/recovery.test.ts`, `src/views/RepoDetailsView.test.tsx`, `src/components/RepoCard.test.tsx`, `src/App.recovery.integration.test.tsx` | (N/A) | (optional subset) | `e2e/recovery.spec.ts` | (optional) |
