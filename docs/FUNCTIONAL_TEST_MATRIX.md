...
| Repo lifecycle (local + SSH) | Wrong URL/path handling; SSH remediation regressions | `src/services/borgService.test.ts`, `src/test/sshHelpers.test.ts` | `src/test/ipcContract.test.ts` | `e2e/repositories.spec.ts` | `e2e/repositories.spec.ts` | (optional) |
