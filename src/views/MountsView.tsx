      ...
      {isCreating && (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6 shadow-lg animate-in fade-in slide-in-from-top-4 duration-300">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
             <div>
               <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2">Source Repository</label>
               <select 
             ...
             <div>
               <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2 flex justify-between">
                   Target Archive
                   {currentRepoStatus === 'connecting' && <span className="text-blue-500 flex items-center gap-1 normal-case font-normal"><Loader2 className="w-3 h-3 animate-spin"/> Updating...</span>}
               </label>
               <select 
               ...
               >
                 {archives.length === 0 ? (
                     <option>No archives found (Connect repo first)</option>
                 ) : (
           ...
           <div className="flex flex-col md:flex-row gap-4 mb-4">
               <div className="flex-1 p-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-700 dark:text-slate-300">
                   ...
                   <div className="grid grid-cols-[120px_1fr] gap-y-1 gap-x-2 text-xs font-mono">
                       ...
                       <span className="text-slate-800 dark:text-slate-200 break-all bg-white dark:bg-slate-800 px-1 rounded border border-slate-200 dark:border-slate-700">{internalPath}</span>
                       <span className="text-slate-500 dark:text-slate-500">Explorer URI:</span>
                       <span className="text-slate-800 dark:text-slate-200 break-all bg-white dark:bg-slate-800 px-1 rounded border border-slate-200 dark:border-slate-700">{explorerPathHint}</span>
                   </div>
               </div>
           ...
           <div className="flex justify-end pt-2 border-t border-gray-100 dark:border-slate-700">
             <Button onClick={handleMount} disabled={!selectedArchive || currentRepoStatus === 'connecting' || archives.length === 0} className="w-full sm:w-auto">
      ...
      <div className="grid grid-cols-1 gap-4">
        {mounts.length === 0 && !isCreating && (
          <div className="flex flex-col items-center justify-center py-16 bg-white dark:bg-slate-800 rounded-xl border-2 border-dashed border-gray-200 dark:border-slate-700 text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 max-w-xs mx-auto">Mount an existing archive to browse its contents like a normal folder.</p>
            <Button onClick={() => setIsCreating(true)} variant="secondary" className="mt-6">
                Mount your first archive
            </Button>
          </div>
        )}
        ...
        {mounts.map(mount => {
          ...
          return (
            <div key={mount.id} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-all flex flex-col sm:flex-row items-center justify-between group gap-4">
              <div className="flex items-center gap-5 w-full sm:w-auto">
                 <div className="w-14 h-14 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl flex-shrink-0 flex items-center justify-center border border-indigo-100 dark:border-indigo-900/50">
                    {isLinuxPath ? (
                        ...
                        <span className="font-bold text-indigo-600 dark:text-indigo-400 text-2xl">{mount.localPath.replace(':', '')}</span>
                    )}
                 </div>
                 <div className="min-w-0 flex-1">
                    <h4 className="font-bold text-slate-800 dark:text-white truncate text-lg" title={mount.archiveName}>{mount.archiveName}</h4>
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 mt-1">
                        <p className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                           <HardDrive className="w-3 h-3 flex-shrink-0" />
                           <span className="font-mono bg-gray-100 dark:bg-slate-700/50 px-1.5 py-0.5 rounded text-slate-600 dark:text-slate-300 border border-gray-200 dark:border-slate-600">{mount.localPath}</span>
                        </p>
                        {repo && (
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto mt-2 sm:mt-0 pt-3 sm:pt-0 border-t sm:border-0 border-gray-100 dark:border-slate-700">
                 <Button variant="secondary" onClick={() => handleOpenFolder(mount.localPath)} className="flex-1 sm:flex-none justify-center dark:bg-slate-700 dark:text-slate-200 dark:border-slate-600 dark:hover:bg-slate-600">
